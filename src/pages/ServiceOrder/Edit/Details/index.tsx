...
const ServiceOrderDetails: React.FC = () => {
  ...
  const sendRequisition = async (): Promise<void> => {
    ...
    await Promise.all(
      requisitions.map(
        async (requisition): Promise<void> => {
          ...
          const { status: responseStatus, mensagem } = response.data;
  ...
  const sendMaterials = async (): Promise<void> => {
    const serviceOrderMaterialRepository = offlineRepository.servicesOrdersMaterials(
      database,
    );

    const serviceOrderMaterials = await serviceOrderMaterialRepository.findByOs(
  ...
  const sendServiceOrder = async (): Promise<void> => {
    const response = await api.post(
  ...
  const handleSendServiceOrder = async (): Promise<void> => {
    const isConnected = await connection.isConnected();
    ...
    <Container>
      ...
      <OsHeader>
        ...
        <OsActions>
          ...
          {/* <OsSendButton
            enabled={!serviceOrder.sent}
            onPress={handleSendServiceOrder}
          >
            {loadingSendOs ? (
              <OsSendButtonText>ENVIANDO...</OsSendButtonText>
            ) : (
              <OsSendButtonText>
                {serviceOrder.sent ? 'OS JA ENVIADA' : 'ENVIAR OS'}
              </OsSendButtonText>
            )}
          </OsSendButton> */}
        </OsActions>
